...
5-1. 무엇이 반정규화의 대상이 되는가?
  ...
  3. 테이블에 지나치게 조인을 많이 사용하게 되어 데이터를 조회하는 것이 기술적으로 어려울 
...
### 트랜잭션의 특성
트랜잭션은 어떠한 특성을 만족해야할까? Transaction은 다음의 ACID라는 4가지 특성을 만족해야 한다.

#### 원자성(Automicity)
만약 트랜잭션 중간에 어떤 문제가 발생한다면 트랜잭션에 해당하는 어떠한 작업 내용도 수정되어서는 안되며, 아무런 문제가 발생되지 않았을 경우에만 모든 작업이 수행되어야 한다. 
